...
\title{Documentation for SimpleTree Plugin}
...
\address{Laboratory of Forest Inventory (LIF)\\
Institut National de l'Information Géographique et Forestière (IGN)\\
11 rue de l'Ile de Corse\\
54000 Nancy\\
jan.hackenberg@nancy.inra.fr}

\begin{abstract}
%% Text of abstract
This file contains a documentation to use the beta release of the SimpleTree Plugin within Computree.


\end{abstract}

\begin{keyword}
SimpleTree \sep QSM \sep Computree
...
\begin{figure}[h]
\centering\includegraphics[width=0.8\linewidth]{figures/simpletree}
\caption{SimpleTree Method}
\end{figure}

This document describes the usage of a plugin of the method available within the SimpleTree software \cite{simpletree} to produce quantitative structure models of laserscanned tress.

The plugin needs to be used properly:
\begin{enumerate}
\item A point cloud cut out in a format accessible with Computree. The cloud has to be an isolated tree. Noise should have been removed. At the moment the de-noising procedures from the SimpleTree stand alone software are not available.
\item The cut above ground of the cloud needs to be done paralell to the x-y plane to ensure best results.
\item Rather than the SimpleTree method this method needs no Input parameters. All parameters are estimated by the software itself. Optimization of those parameters is user hidden. 
\item To run the method into a pipeline one has to put a comma separated value file of the form presented in Table \ref{paramter_tab}. No header included.
\end{enumerate}
...
\hline
\textbf{Tree ID} & \textbf{Output path} & \textbf{species}& \textbf{GT}  &  \textbf{Switch}\\
\hline
Q01.asc & D:/Data/output/temp & \textit{Quercus\_petraea} & 0.722 & TRUE \\
Ery\_01.asc  & D:/Data/output/temp & \textit{Erythrophleum\_fordii} & 0.405 & FALSE\\
Pinus01.asc  &D:/Data/output/temp & \textit{Pinus\_massoiana} & 0.865
 & TRUE \\
\hline
\end{tabular}
...
Those parameters describe:

\begin{enumerate}
\item \textbf{Tree ID} The file name of the loaded cloud. This one is used to make an allignment between the output model and the point cloud.
\item \textbf{Output path} A string containing the path to the output folder
\item \textbf{species} The species name. This one is included in the output file as well. Later you will be able to separate by species within R.
\item \textbf{GT} The Ground Truth volume in [m$^3$]. At the moment this one is not used, but I plan to include to make the error of pipelined tree models visible by this. Can also be set to zero or another number if not known.
\item \textbf{Switch} Activate with TRUE if your cloud is both of high quality and the twigs are lost during leave de-noising. In this case no allometry is performed. 
\end{enumerate}
